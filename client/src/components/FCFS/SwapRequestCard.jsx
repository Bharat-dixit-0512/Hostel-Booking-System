...
const buildSwapReferenceId = ({ type, sender, currentRoom, targetRoom }) => {
  const referenceSeed = `${type}|${sender || ""}|${currentRoom || ""}|${targetRoom || ""}`;
  let hash = 0;

  for (let index = 0; index < referenceSeed.length; index += 1) {
    hash = (hash << 5) - hash + referenceSeed.charCodeAt(index);
    hash |= 0;
  }

  return `SWP-${(Math.abs(hash) % 9000) + 1000}`;
};
...
const SwapRequestCard = ({
  ...
  const referenceId = buildSwapReferenceId({
    type,
    sender,
    currentRoom,
    targetRoom,
  ...
  return (
    <div className="bg-[#15202b] border border-white/5 rounded-4xl p-8 space-y-8 shadow-2xl relative overflow-hidden">
      ...
      <div className="flex items-center gap-4 relative z-10">
        <div
          className={`p-3 rounded-2xl ${isIncoming ? "bg-blue-500/10 text-blue-500" : "bg-purple-500/10 text-purple-500"}`}
        >
      ...
      {isIncoming ? (
        ...
        <div className="py-3 text-center border-t border-white/5">
          <p className="text-[10px] font-bold text-slate-500 animate-pulse">
