...
const FCFSAnalytics = () => {
  ...
  const logs = [
    ...
    },
    {
      ...
      user: "Sneha Kapoor",
      time: "10:00:00.089",
      action: "Room 101 Locked (Rejected)",
      status: "conflict",
    },
    {
      id: 3,
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-slate-500 text-sm font-medium">
              Monitoring First-Come-First-Serve traffic and race conditions.
            </p>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="flex items-center justify-between mb-10">
              <div>
                <h3 className="text-xl font-bold text-white flex items-center gap-2">
                  <Flame size={20} className="text-orange-500" /> Inventory
                  Heatmap
                </h3>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-1">
                  Kailash Bhavan - All Blocks
                </p>
              </div>
              <div className="flex gap-2">
                <div className="flex items-center gap-2 px-3 py-1.5 bg-red-500/10 rounded-lg text-[9px] font-black text-red-500 border border-red-500/20">
                  HOT
                </div>
                <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-500/10 rounded-lg text-[9px] font-black text-emerald-500 border border-emerald-500/20">
                  FREE
                </div>
              </div>
            </div>
            ...
            <div className="space-y-8">
              {[1, 2, 3].map((floor) => (
                <div key={floor} className="space-y-3">
                  <h4 className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] ml-1">
                    Floor 0{floor}
                  </h4>
                  <div className="grid grid-cols-5 md:grid-cols-10 gap-3">
                    {[...Array(10)].map((_, i) => {
                      ...
                      const isHot =
                        roomNo < 105 || roomNo === 204 || roomNo === 301;
                      return (
                        <div
                          key={i}
                        ...
                        >
                          ...
                          {isHot && (
                            <div className="w-1 h-1 rounded-full bg-red-500 mt-1 animate-pulse" />
                          )}
                        </div>
                      );
                    })}
          ...
          <div className="lg:col-span-5 bg-[#15202b]/40 border border-white/5 rounded-[40px] p-10 backdrop-blur-xl">
            <div className="flex items-center justify-between mb-10">
              <div>
                <h3 className="text-xl font-bold text-white flex items-center gap-2">
                  <ShieldCheck size={20} className="text-blue-500" /> Fairness
                  Audit
                </h3>
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mt-1">
            ...
            <div className="space-y-4">
              {logs.map((log) => (
                <div
                  key={log.id}
                  className="group flex items-center justify-between p-4 bg-black/30 rounded-2xl border border-white/5 hover:border-white/10 transition-all"
                >
            ...
            <div className="mt-8 p-6 bg-blue-600/5 border border-blue-500/10 rounded-3xl">
              <p className="text-[9px] text-slate-500 leading-relaxed font-medium">
                * Logs are generated with millisecond precision to resolve FCFS
                race condition disputes. High-velocity collisions are
                automatically prioritized by the system core timestamp.
              </p>
            </div>
          </div>
        </div>
