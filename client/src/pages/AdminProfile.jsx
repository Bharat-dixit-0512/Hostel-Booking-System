...
import { 
  User, Hash, Building2, Briefcase, 
  ShieldCheck, Mail, ShieldAlert, ChevronRight, Activity, FileText
} from 'lucide-react';
...
const AdminProfile = () => {
  ...
  const [profile] = useState({
    name: "Dr. Alok Kumar",
    employee_id: "EMP-GLA-9921",
    email: "alok.admin@gla.ac.in",
    department: "Hostel Management Cell",
    role: "Chief Warden / Senior Admin",
  ...
  const handleExportLogs = () => {
    playClickSound();
    toast.success("Administrative access logs exported");
  };
            ...
            <div className="bg-[#15202b]/40 border border-white/5 rounded-4xl backdrop-blur-xl overflow-hidden shadow-2xl">
              ...
              <div className="px-8 pb-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <AdminInfoTile icon={<User />} label="Full Name" value={profile.name} />
                  <AdminInfoTile icon={<Hash />} label="Employee ID" value={profile.employee_id} color="text-emerald-500" />
                  <AdminInfoTile icon={<Mail />} label="Admin Email" value={profile.email} />
                  <AdminInfoTile icon={<Briefcase />} label="Department" value={profile.department} />
                  <AdminInfoTile icon={<ShieldAlert />} label="Access Level" value={profile.access_level} />
                  <AdminInfoTile icon={<Activity />} label="Account Status" value="Active" color="text-emerald-400" />
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="p-8">
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mt-2">Privileged</p>
                  </div>
                </div>
                <div className="space-y-3">
                  <AdminActionBtn label="System Settings" onClick={playClickSound} />
                  <AdminActionBtn label="Audit Trails" onClick={playClickSound} />
                  <AdminActionBtn label="Permission Matrix" onClick={playClickSound} />
                </div>
              </div>
            </AnimatedBorder>

            <div className="bg-emerald-500/5 border border-emerald-500/10 p-6 rounded-3xl text-center backdrop-blur-md">
              <p className="text-[10px] text-emerald-600 font-bold uppercase tracking-[0.3em]">Administrative Console</p>
            </div>
          </div>
...
const AdminInfoTile = ({ icon, label, value, color = "text-white" }) => (
  <div className="p-4 rounded-2xl bg-[#0b1118]/40 border border-white/5 flex items-center gap-4 hover:border-emerald-500/40 hover:bg-[#0b1118]/60 transition-all group cursor-pointer">
