...
export const syncHostelStudentFromAuthStudent = async (
  authStudent,
  session = null,
) => {
...
export const ensureStudentIsEligibleForHostel = async ({
  hostelId,
  student,
  session = null,
}) => {
...
export const createPendingStudentBooking = async ({
  ...
  try {
    ...
    if (hostelStudent.room_allocated) {
      throw new ApiError(
        409,
        "Student already has a confirmed room allocation",
      );
    }
    ...
    if (roomPrice === undefined) {
      throw new ApiError(
        409,
        "Pricing is not configured for the selected room category",
      );
    }
...
export const createOfflineBooking = async ({
  ...
  if (authStudent.residence !== RESIDENCE_TYPES.HOSTLER) {
    throw new ApiError(
      403,
      "Only hostler students can receive hostel bookings",
    );
  }
  ...
  try {
    ...
    const hostelStudent = await syncHostelStudentFromAuthStudent(
      authStudent,
      session,
    );
    ...
    const activeBooking = await getStudentActiveBooking(
